...
function getStoreImage(store: Store): string {
  ...
  if (FALLBACK_IMAGES[store.name]) return FALLBACK_IMAGES[store.name];
  if (store.name.toLowerCase().includes("oasis")) return "/store/oasis-ajman.webp";
  if (store.city === "Dubai") return "/store/Mirdif-Dubai.webp";
  if (store.name.toLowerCase().includes("khan")) return "/store/Al-Khan-Sharjah.webp";
  if (store.city === "Sharjah") return "/store/Al-Taawun-Sharjah.webp";
  return "/store/Al-Nuaimia-Ajman.webp";
}
...
export default function StoresPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/stores")
      .then(r => r.json())
      .then(d => { setStores(d.stores || []); setLoadingStores(false); })
  ...
  const cityLabels: Record<string, string> = {
    all: t("filter_all"), Dubai: t("filter_dubai"),
    Sharjah: t("filter_sharjah"), Ajman: t("filter_ajman"),
  };
  const cityConfig: Record<string, { bg: string; color: string }> = {
    Dubai:   { bg: "#f0f4f8", color: "#1a2e3f" },
    Sharjah: { bg: "#faf6f2", color: "#c8956c" },
    Ajman:   { bg: "#f0f4f8", color: "#2c4a63" },
  };
  ...
  const schemaData = {
    ...
    itemListElement: stores.map((store, i) => ({
      ...
      item: {
        "@type": "GroceryStore",
        name: `Trolleys Supermarket — ${store.name}`,
        image: store.image ? `https://trolleyssupermarketllc.com${store.image}` : undefined,
        address: { "@type": "PostalAddress", streetAddress: store.address, addressLocality: store.city, addressCountry: "AE" },
        telephone: store.phone,
        openingHours: store.hours,
        geo: { "@type": "GeoCoordinates", latitude: store.lat, longitude: store.lng },
        hasMap: store.maps,
        url: `https://trolleyssupermarketllc.com/${locale}/stores/${storeSlugMap[store.name] ?? ""}`,
      },
    })),
  };
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .stores-card { background: #fff; border: 1px solid #f0ebe4; border-radius: 14px; transition: all .3s cubic-bezier(.25,.46,.45,.94); cursor: pointer; flex-shrink: 0; }
        .stores-card:hover { transform: translateY(-2px); box-shadow: 0 8px 28px rgba(0,0,0,.05), 0 4px 10px rgba(0,0,0,.03); }
        .stores-card.active { border-color: #c8956c !important; box-shadow: 0 0 0 2px rgba(200,149,108,.15) !important; }
        .stores-card-img { transition: transform .5s cubic-bezier(.25,.46,.45,.94); }
        .stores-card:hover .stores-card-img { transform: scale(1.06); }
        .stores-filter-btn { padding: 7px 18px; border-radius: 999px; font-size: 11.5px; font-weight: 600; border: none; cursor: pointer; transition: all .25s; }
        ...
        .stores-action-btn { display: flex; align-items: center; justify-content: center; gap: 5px; flex: 1; padding: 9px 8px; border-radius: 8px; font-size: 11.5px; font-weight: 600; text-decoration: none; border: none; cursor: pointer; transition: all .2s; }
        .stores-action-btn:hover { transform: translateY(-1px); filter: brightness(1.08); }
        .stores-details-btn { display: flex; align-items: center; justify-content: center; gap: 5px; width: 100%; margin-top: 8px; padding: 7px 12px; border-radius: 8px; font-size: 11.5px; font-weight: 600; color: #1C75BC !important; text-decoration: none; border: 1px solid #dbeafe; background: #eff8ff; transition: all .2s; }
        .stores-details-btn:hover { background: #1C75BC; color: #fff !important; border-color: #1C75BC; }
        .stores-search { width: 100%; border: 1.5px solid #f0ebe4; border-radius: 10px; padding: 10px 14px; font-size: 13px; outline: none; font-family: inherit; transition: all .25s; background: #fdfbf9; color: #4a4a4a; }
        ...
        .stores-list-scroll { overflow-y: auto; overflow-x: hidden; scrollbar-width: thin; scrollbar-color: #e0d5c5 transparent; padding-right: 4px; }
        ...
        @media(min-width: 1025px) {
          ...
          .stores-map-col { width: 58% !important; position: sticky !important; top: 80px !important; height: 580px !important; }
          .stores-list-col { width: 42% !important; display: flex !important; flex-direction: column !important; max-height: 580px !important; }
        ...
        @media(max-width: 1024px) {
          ...
          .stores-list-col { width: 100% !important; max-height: none !important; }
      ...
      <div className="cp" style={{ background: "#fdfbf9", minHeight: "100vh", direction: isRTL ? "rtl" : "ltr" }}>
        ...
        {/* Header */}
        <div style={{ background: "#1C75BC", position: "relative", overflow: "hidden", padding: "48px 32px 52px" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "32px clamp(16px,3vw,32px) 80px" }}>
          {loadingStores ? (
            <div style={{ textAlign: "center", padding: "80px 0", color: "#94a3b8" }}>
              <div style={{ width: 36, height: 36, border: "3px solid #e2e8f0", borderTopColor: "#1C75BC", borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 16px" }} />
              <p style={{ fontSize: 14 }}>{isRTL ? "جارٍ التحميل..." : "Loading stores..."}</p>
            </div>
          ) : (
            <div className="stores-layout">
              ...
              <div className="stores-map-col">
                ...
                <p className="map-zoom-hint" style={{ fontSize: 10.5, color: "#a0a0a0", textAlign: "center", marginTop: 8 }}>{isRTL ? "Ctrl + التمرير للتكبير" : "Ctrl + scroll to zoom"}</p>
                <p className="map-touch-hint" style={{ fontSize: 10.5, color: "#a0a0a0", textAlign: "center", marginTop: 8 }}>{isRTL ? "استخدم إصبعين للتكبير" : "Use two fingers to zoom"}</p>
              </div>

              {/* List */}
              <div className="stores-list-col">
                <div className="stores-card" style={{ padding: 16, cursor: "default", marginBottom: 12 }}>
                ...
                <div className="stores-list-scroll" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {filteredStores.map(store => {
                    ...
                    return (
                      <div key={store.name} onClick={() => setActiveStore(store.name)} className={`stores-card ${activeStore === store.name ? "active" : ""}`}>
                        <div style={{ display: "flex" }}>
                          ...
                          <div style={{ flex: 1, padding: "14px 14px 12px", minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8, marginBottom: 8 }}>
                              <h3 style={{ fontSize: 13.5, fontWeight: 700, color: "#1a1a1a", margin: 0, lineHeight: 1.3 }}>{store.name}</h3>
                            ...
                            <p style={{ fontSize: 11.5, color: "#7a7a7a", marginBottom: 4, display: "flex", alignItems: "center", gap: 5 }}><MapIcon /> {store.address}</p>
                            <p style={{ fontSize: 11.5, color: "#7a7a7a", marginBottom: 12, display: "flex", alignItems: "center", gap: 5 }}><ClockIcon /> {store.hours}</p>
                            <div style={{ display: "flex", gap: 6 }}>
                              <a href={"tel:" + store.phone} className="stores-action-btn" style={{ background: "#1C75BC", color: "#fff" }} onClick={e => e.stopPropagation()}><PhoneIcon /> {t("call")}</a>
                              <a href={"https://wa.me/" + store.whatsapp} target="_blank" rel="noopener noreferrer" className="stores-action-btn" style={{ background: "#25d366", color: "#fff" }} onClick={e => e.stopPropagation()}><WAIcon /> WhatsApp</a>
                              <a href={store.maps} target="_blank" rel="noopener noreferrer" className="stores-action-btn" style={{ flex: "0 0 36px", background: "#faf6f2", color: "#c8956c", border: "1px solid #f0e4d8" }} onClick={e => e.stopPropagation()} title={t("directions")}><MapIcon /></a>
                            </div>
                            {slug && (
                  ...
                  {filteredStores.length === 0 && (
                    <div className="stores-card" style={{ textAlign: "center", padding: "48px 0", cursor: "default" }}>
                      <p style={{ fontSize: 14, color: "#a0a0a0", margin: 0 }}>{isRTL ? "لا توجد فروع متطابقة" : "No matching stores found"}</p>
                    </div>
                  )}
                </div>
              </div>
